...
def init():
    URL = "https://www.hostpinnacle.co.ke/clients/cart.php?a=add&domain=register"
    page = requests.get(URL)

    soup = BeautifulSoup(page.content, "html.parser")

    # STEP 1: Extract 'token'
    # soup.find_all(attrs={"name": "token"})
    input_token = soup.find("input", attrs={"name": "token"})
    if input_token:
        global token
...
def scrape_hostpinnacle_for_domains(domain):
    if not token:
        init()

    # domain = 'test.com'
    url = 'https://www.hostpinnacle.co.ke/clients/index.php'
    params = {
    ...
    headers = {
        # 'authority': 'www.hostpinnacle.co.ke',
        # 'method': 'POST',
        # 'path': '/clients/index.php?rp=/domain/check',
        # 'scheme': 'https',
        # 'Accept': '*/*',
